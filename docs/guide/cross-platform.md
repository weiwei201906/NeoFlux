...
:::tip
On mobile, pass the platform surface (`ANativeWindow*` on Android,
`CAMetalLayer*`/`CAEAGLLayer*` on iOS) as the `platform_surface` parameter to
`Application::Init`. On desktop, pass `nullptr` and GLFW creates the window
automatically.
:::
