...
//
// Unit tests for the SPSC lock-free ring queue. Uses the explicit
// instantiations provided by src/core/ring_queue.cpp (int, uint64_t,
// std::unique_ptr<int>, RenderCommand).
// =============================================================================
...
TEST(SpscRingQueueTest, MoveOnlyType) {
  SpscRingQueue<std::unique_ptr<int>> queue(8);
  EXPECT_TRUE(queue.TryPush(std::make_unique<int>(99)));
