...
TEST(BackpressureStress, SingleFrameOverflow) {
  ...
  for (RenderCommand& command : commands) {
    command.type = RenderCommandType::kDrawRect;
    command.rect = Rect{.x = 0.0F, .y = 0.0F, .width = 10.0F, .height = 10.0F};
  }
...
TEST(BackpressureStress, PartialFillThenOverflow) {
  ...
  for (auto& c : batch1) {
    c.type = RenderCommandType::kSave;
  }
  ...
  for (auto& c : batch2) {
    c.type = RenderCommandType::kRestore;
  }
...
TEST(BackpressureStress, LargeQueueNoOverflow) {
  ...
  for (std::size_t i = 0; i < commands.size(); ++i) {
    commands.at(i).type = (i & 1U) != 0U ? RenderCommandType::kDrawRect
...
TEST(BackpressureStress, ConcurrentProducerConsumerOverflow) {
  ...
  // Final drain: the consumer may exit while the producer is mid-push, so
  // drain once more after both threads are joined to catch any stragglers.
  int value;
