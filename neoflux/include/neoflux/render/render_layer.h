...
class RenderLayer : public NonCopyable {
 public:
  ...
  // Registers a media player whose GL texture must be updated every frame.
  // The render thread calls InitRender() once and UpdateTexture() each frame
  // with a current GL context. The player must outlive the registration;
  // call UnregisterMediaPlayer() before destroying it.
  void RegisterMediaPlayer(MediaPlayer* player);

  // Unregisters a media player previously registered with
  // RegisterMediaPlayer(). No-op if the player is not registered.
  void UnregisterMediaPlayer(MediaPlayer* player);
 ...
 private:
  ...
  // Registered media players whose textures are refreshed on the render
  // thread. Guarded by media_players_mutex_; players are raw (not owned).
  std::vector<MediaPlayer*> media_players_{};
  std::mutex media_players_mutex_{};
};
