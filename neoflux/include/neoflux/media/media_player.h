...
class MediaPlayer {
 public:
  ...
  // Returns the most recently updated GL texture name. Thread-safe: the
  // render thread writes it inside UpdateTexture(); the UI thread reads it
  // when composing draw commands. Returns 0 if no frame is available yet.
  [[nodiscard]] virtual std::uint32_t GetTextureId() const noexcept = 0;
};
