...
//
// All method implementations are in src/native/desktop/mpv_media_player.cpp.
// =============================================================================

#ifndef NEOFLUX_NATIVE_DESKTOP_MPV_MEDIA_PLAYER_H_
#define NEOFLUX_NATIVE_DESKTOP_MPV_MEDIA_PLAYER_H_
...
class MpvMediaPlayer final : public MediaPlayer {
 public:
  ...
  [[nodiscard]] std::uint32_t GetTextureId() const noexcept override;
 ...
 private:
  ...
  // GL texture name. Written by the render thread (UpdateTexture), read by
  // the UI thread (GetTextureId); atomic for cross-thread visibility.
  std::atomic<std::uint32_t> texture_id_{0};
  int video_width_ = 0;
  int video_height_ = 0;
  double volume_ = 1.0;
...
#endif  // NEOFLUX_NATIVE_DESKTOP_MPV_MEDIA_PLAYER_H_
