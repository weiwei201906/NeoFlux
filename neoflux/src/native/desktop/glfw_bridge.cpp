...
class Win32NativeTextField final : public NativeTextField {
 public:
  explicit Win32NativeTextField(GLFWwindow* window)
      : window_(window),
        control_id_(next_control_id_++),
        hwnd_(glfwGetWin32Window(window)) {
    // Subclass the GLFW window once to intercept WM_COMMAND.
    if (g_original_wndproc == nullptr) {
      // SetWindowLongPtr returns a LONG_PTR handle; converting it back to
      // WNDPROC is the documented Win32 idiom (false positive for
    ...
    // widget's focus border. HMENU carries a control id for WM_COMMAND
    // routing -- the documented Win32 idiom (false positive for
    // performance-no-int-to-ptr).
    edit_hwnd_ = CreateWindowExW(
        0, L"EDIT", L"",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | WS_BORDER | ES_LEFT,
        0, 0, 100, 24, hwnd_,
        // NOLINTNEXTLINE(performance-no-int-to-ptr) - HMENU carries the control id.
...
GlfwBridge::~GlfwBridge() {
  // Destructor must not throw; Shutdown() logs via glog which may allocate.
  try {
    Shutdown();
  } catch (...) {
...
void GlfwBridge::KeyCallback(GLFWwindow* window, int key,
  ...
  // KeyEvent is value-initialized with the explicit unknown sentinel so no
  // invalid enum value (0) is produced by default member init.
  KeyEvent event{.key = KeyCode::kUnknown, .modifiers = 0, .pressed = false};
