...
std::uint32_t MpvMediaPlayer::UpdateTexture() {
  ...
  std::uint32_t texture_id = texture_id_.load();
  ...
  if (texture_id == 0) {
    ...
    gl.BindTexture(kGlTexture2d, texture_id);
  ...
  if (frame_callback_ != nullptr) {
    frame_callback_(texture_id, width, height);
  }

  return texture_id;
}

std::uint32_t MpvMediaPlayer::GetTextureId() const noexcept {
...
std::uint32_t MpvMediaPlayer::GetTextureId() const noexcept { return 0; }
void MpvMediaPlayer::PollEvents() {}
void MpvMediaPlayer::SetPropertyDouble(const char* name, double value) {
  (void)name;
  (void)value;
