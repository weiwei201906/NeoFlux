// =============================================================================
// NeoFlux - gl_renderer.cpp
//
// OpenGL fallback renderer implementation. Loads GL entry points at runtime
// (glfwGetProcAddress on desktop, eglGetProcAddress on Android, dlsym on
// iOS), compiles a single shader pair, and renders rectangles / UTF-8 text /
// external textures through a shared VBO. FreeType rasterizes glyphs into a
// 1024x1024 grayscale atlas texture.
//
// All methods must be called from the render thread with a current GL
// context (see RenderLayer::RenderLoop).
// =============================================================================
...
using GlFloat = float;
using GlVoid = void;
// ptrdiff_t matches GLsizeiptr/GLintptr width on all supported platforms.
using GlSizeiptr = std::ptrdiff_t;
using GlIntptr = std::ptrdiff_t;

// ---------------------------------------------------------------------------
// OpenGL function loader. Loads all required entry points via the platform
// resolver after a context is current.
// ---------------------------------------------------------------------------
struct GlLoader {
  ...
  // Resolves the platform GL function resolver.
  //   Desktop: glfwGetProcAddress
  //   Android: eglGetProcAddress
  //   iOS:     dlsym(RTLD_DEFAULT, ...)
  bool Load() {
...
// Converts an 8-bit color channel to the normalized [0,1] float used by GL.
// The explicit cast avoids implicit int->float narrowing (clang-tidy).
constexpr float ColorChannel(std::uint8_t channel) noexcept {
  return static_cast<float>(channel) * kColorScale;
}
...
// GL constants (avoid including GL headers which conflict with the custom
// loader). Values match the OpenGL / OpenGL ES specification.
...
constexpr GlEnum kGlTriangleFan = 0x0006;
...
constexpr GlEnum kGlColorBufferBit = 0x00004000U;
constexpr GlEnum kGlDepthBufferBit = 0x00000100U;
constexpr GlEnum kGlBlendSrcAlpha = 0x0302;
constexpr GlEnum kGlBlendOneMinusSrcAlpha = 0x0303;
...
constexpr GlEnum kGlTextureMinFilter = 0x2800;
constexpr GlEnum kGlTextureMagFilter = 0x2801;
...
constexpr GlEnum kGlDynamicDraw = 0x88E8;
...
// ---------------------------------------------------------------------------
// PImpl: all GL / FreeType state lives here so the header stays dependency
// free. All methods run on the render thread (GL context current).
// ---------------------------------------------------------------------------
struct GlRendererImpl::Impl {
...
#else
    // Mobile: the surface size is managed by the platform bridge.
    gl.glViewport(0, 0, width_, height_);
#endif
    gl.glClearColor(ColorChannel(clear.r), ColorChannel(clear.g),
                    ColorChannel(clear.b), ColorChannel(clear.a));
    gl.glClear(kGlColorBufferBit | kGlDepthBufferBit);
    // Update u_resolution every frame so window resizes take effect without
    // re-initializing GL. u_resolution uses logical (layout) size, not
  ...
  void EndFrame() { /* buffer swap handled by platform bridge */ }
  ...
  void DrawRect(const Rect& rect, const Color& color) {
    ...
    gl.glUniform4f(u_color_, ColorChannel(color.r), ColorChannel(color.g),
    ...
    BindBuffer(vbo_);
    gl.glBufferSubData(kGlArrayBuffer, 0, sizeof(vertices), vertices);
    gl.glDrawArrays(kGlTriangles, 0, 6);
  }

  // Draws a filled rounded rectangle using a triangle fan. The boundary is
  // sampled at kSeg points per corner; interior is filled from the rectangle
  // centre. Coordinates use y-down (screen space), so the y-component of each
  // arc uses -sin(angle).
  void DrawRoundedRect(const Rect& rect, const Color& color, float radius) {
    ...
    // 1 centre + 4 corners * kSeg boundary points (last point of each corner
    // is shared with first point of next, so we skip the duplicate on
    // corners 1-3 and add a final closing point).
    ...
    BindBuffer(vbo_);
    gl.glBufferSubData(kGlArrayBuffer, 0, sizeof(vertices), vertices);
    gl.glDrawArrays(kGlTriangleFan, 0, kVertexCount);
  }
  ...
  void DrawText(std::string_view text, const Point& position,
    ...
    gl.glActiveTexture(kGlActiveTexture0);
    ...
    while (i < text.size()) {
      std::uint32_t cp = 0;
      // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - loop bound guarantees i < text.size().
      ...
      for (int j = 1; j < seq_len && i + j < text.size(); ++j) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - j is bounded by seq_len and the size check.
  ...
  void ApplyClip(const Rect& rect) const {
    if (rect.width < 0.0F || rect.height < 0.0F) {
      gl.glDisable(kGlScissorTest);
      return;
    }
    gl.glEnable(kGlScissorTest);
  ...
  bool InitializeGL() {
    ...
    const std::string vert_src = std::string(kGlVersion) + kVertBody;
    const std::string frag_src = std::string(kGlVersion) + kFragBody;
    const char* vert_ptr = vert_src.c_str();
    const char* frag_ptr = frag_src.c_str();

    const GlUint vs = gl.glCreateShader(kGlVertexShader);
    const GlUint fs = gl.glCreateShader(kGlFragmentShader);
    ...
    GlInt ok = 0;
    gl.glGetShaderiv(vs, kGlCompileStatus, &ok);
    if (ok == 0) {
      char log[512];
    ...
    gl.glGetShaderiv(fs, kGlCompileStatus, &ok);
    if (ok == 0) {
      char log[512];
    ...
    gl.glGetProgramiv(program_, kGlLinkStatus, &ok);
    if (ok == 0) {
      char log[512];
    ...
    gl.glBindBuffer(kGlArrayBuffer, vbo_);
    ...
    gl.glBufferData(kGlArrayBuffer, kMaxVboBytes, nullptr, kGlDynamicDraw);
    gl.glEnableVertexAttribArray(0);
    gl.glVertexAttribPointer(0, kVertexSize, kGlFloat, 0,
                             kVertexSize * static_cast<GlSizei>(sizeof(float)),
                             nullptr);

    gl.glGenTextures(1, &atlas_texture_);
    gl.glBindTexture(kGlTexture2d, atlas_texture_);
    gl.glTexParameteri(kGlTexture2d, kGlTextureMinFilter,
                       static_cast<GlInt>(kGlLinear));
    gl.glTexParameteri(kGlTexture2d, kGlTextureMagFilter,
                       static_cast<GlInt>(kGlLinear));
    gl.glTexParameteri(kGlTexture2d, kGlTextureWrapS,
                       static_cast<GlInt>(kGlClampToEdge));
    gl.glTexParameteri(kGlTexture2d, kGlTextureWrapT,
                       static_cast<GlInt>(kGlClampToEdge));
    // GL_R8 internal format: 8-bit single-channel grayscale for glyph coverage.
    gl.glTexImage2D(kGlTexture2d, 0, static_cast<GlInt>(kGlR8), kAtlasSize,
                    kAtlasSize, 0, kGlRed, kGlUnsignedByte, nullptr);
    // Clear the atlas to zero (transparent). glTexImage2D with nullptr leaves
    // the texture contents undefined, which can cause garbage pixels (white
    // squares) on first frame before any glyph is uploaded.
    {
      std::vector<unsigned char> zeros(
          static_cast<std::size_t>(kAtlasSize) * kAtlasSize, 0);
      gl.glTexSubImage2D(kGlTexture2d, 0, 0, 0, kAtlasSize, kAtlasSize,
                         kGlRed, kGlUnsignedByte, zeros.data());
    }

    gl.glEnable(kGlBlend);
    gl.glBlendFunc(kGlBlendSrcAlpha, kGlBlendOneMinusSrcAlpha);
  ...
  const GlyphInfo* GetGlyph(FT_Face face, std::uint32_t codepoint,
    ...
    if (pitch >= 0) {
      for (int row = 0; row < h; ++row) {
        // Use explicit size_t arithmetic to avoid implicit int->ptrdiff_t
        // widening; parens clarify operator precedence.
        const auto src_offset = static_cast<std::size_t>(row) *
                                static_cast<std::size_t>(pitch);
        const auto dst_offset = static_cast<std::size_t>(row) *
                                static_cast<std::size_t>(w);
        std::copy_n(bitmap.buffer + src_offset, w, packed.data() + dst_offset);
      }
    } else {
      // Negative pitch: bitmap is bottom-up. Read from bottom row.
      for (int row = 0; row < h; ++row) {
        const auto src_offset =
            static_cast<std::size_t>(h - 1 - row) *
            static_cast<std::size_t>(-pitch);
        const auto dst_offset = static_cast<std::size_t>(row) *
                                static_cast<std::size_t>(w);
        std::copy_n(bitmap.buffer + src_offset, w, packed.data() + dst_offset);
    ...
    BindTexture(atlas_texture_);
    gl.glPixelStorei(kGlUnpackAlignment, 1);
    gl.glTexSubImage2D(kGlTexture2d, 0, atlas_x_, atlas_y_, w, h, kGlRed,
                       kGlUnsignedByte, packed.data());
    gl.glPixelStorei(kGlUnpackAlignment, 4);  // reset to default
  ...
  // Releases all GL and FreeType resources. Must be called from the render
  ...
  // Cached GL state to avoid redundant state-change calls (each gl* call may
  // trigger an ioctl into the GPU driver, ~33% of CPU in profiling).
  ...
  void BindTexture(GlUint texture) {
    if (texture != current_texture_) {
      gl.glBindTexture(kGlTexture2d, texture);
  ...
  void BindBuffer(GlUint vbo) {
    if (vbo != current_vbo_) {
      gl.glBindBuffer(kGlArrayBuffer, vbo);
      current_vbo_ = vbo;
    }
  }
...
GlRendererImpl::GlRendererImpl() : impl_(std::make_unique<Impl>()) {}
...
bool GlRendererImpl::Init(int width, int height, std::string_view font_dir,
                          void* native_handle) {
  return impl_->Init(width, height, font_dir, native_handle);
}

void GlRendererImpl::BeginFrame(const Color& clear_color) {
  impl_->BeginFrame(clear_color);
}

void GlRendererImpl::EndFrame() { impl_->EndFrame(); }

void GlRendererImpl::DrawRect(const Rect& rect, const Color& color) {
  impl_->DrawRect(rect, color);
}

void GlRendererImpl::DrawRoundedRect(const Rect& rect, const Color& color,
                                     float radius) {
  impl_->DrawRoundedRect(rect, color, radius);
}

void GlRendererImpl::DrawText(std::string_view text, const Point& position,
                              const Color& color, float font_size,
                              std::string_view font_name) {
  impl_->DrawText(text, position, color, font_size, font_name);
}

void GlRendererImpl::DrawTexture(std::uint32_t texture_id, const Rect& rect) {
  impl_->DrawTexture(texture_id, rect);
}

void GlRendererImpl::Save() { impl_->Save(); }

void GlRendererImpl::Restore() { impl_->Restore(); }

void GlRendererImpl::Translate(float delta_x, float delta_y) {
  impl_->Translate(delta_x, delta_y);
}

void GlRendererImpl::ClipRect(const Rect& rect) { impl_->ClipRect(rect); }

void GlRendererImpl::Resize(int width, int height) {
