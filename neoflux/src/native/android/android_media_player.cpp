...
bool AndroidMediaPlayer::CreateSurfaceTexture() {
  ...
  jobject local_st = env->NewObject(surface_texture_class_, ctor, texture_id);
  if (local_st == nullptr) {
  ...
  LOG(INFO) << "AndroidMediaPlayer: SurfaceTexture created (tex=" << texture_id
            << ")";
  return true;
}
...
std::uint32_t AndroidMediaPlayer::UpdateTexture() {
  ...
  const std::uint32_t texture_id = texture_id_.load();
  if (frame_callback_ != nullptr) {
    frame_callback_(texture_id, video_width_, video_height_);
  }

  return texture_id;
}

std::uint32_t AndroidMediaPlayer::GetTextureId() const noexcept {
  return texture_id_.load();
}
