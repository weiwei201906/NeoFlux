...
std::uint32_t IosMediaPlayer::GetTextureId() const noexcept {
...
std::uint32_t IosMediaPlayer::UpdateTextureFromPixelBuffer() {
  ...
  if (ret == kCVReturnSuccess && gl_texture != nullptr) {
    const std::uint32_t texture_id = CVOpenGLESTextureGetName(gl_texture);
    ...
    if (frame_callback_ != nullptr) {
      frame_callback_(texture_id, texture_width_, texture_height_);
    }
  }
