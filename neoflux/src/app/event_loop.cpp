...
void EventLoop::SetTargetFps(int fps) noexcept {
  if (fps <= 0) {
    // RAW_LOG does not allocate, keeping this noexcept function exception-free.
    RAW_LOG(WARNING, "Invalid target FPS: %d, using 60", fps);  // NOLINT(cppcoreguidelines-avoid-do-while): glog macro expands to do-while(0)
    target_fps_ = 60;
    return;
  }
  target_fps_ = fps;
...
void EventLoop::Schedule(Task<void> task) {
  ...
  {
    std::scoped_lock<std::mutex> lock(coroutine_mutex_);
