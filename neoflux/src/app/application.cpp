...
Application::~Application() {
  // Destructor must not throw. Stop() performs best-effort teardown and any
  // exception is swallowed (glog shutdown can allocate during log flush).
  try {
    Stop();
  } catch (...) {
    ...
    (void)0;  // Best-effort teardown: ignore exceptions during destruction.
  }
}
...
bool Application::Init(int argc, char** argv, int window_width,
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    } else if (arg.starts_with("--logtostderr=")) {
      FLAGS_logtostderr =
          (arg == "--logtostderr=true" || arg == "--logtostderr=1");
    } else if (arg.starts_with("--log_dir=") ||
               arg.starts_with("-log_dir=")) {
  ...
  if (auto* bridge = render_layer_->GetPlatformBridge(); bridge != nullptr) {
    ...
    auto* glfw = dynamic_cast<GlfwBridge*>(bridge);
    if (glfw != nullptr) {
...
void Application::SetFontDir(std::string_view dir) noexcept {
  // std::string assignment may allocate; swallow OOM on the noexcept path.
  try {
    font_dir_ = std::string(dir);
  } catch (...) {
    // NOLINTNEXTLINE(bugprone-empty-catch) - swallowing is intentional:
    // SetFontDir is noexcept and allocation failure is unrecoverable here.
    (void)0;  // Keep the noexcept contract: ignore allocation failures.
  }
}
...
void Application::DispatchScrollEvent(
  ...
  auto* bridge =
      dynamic_cast<GlfwBridge*>(render_layer_->GetPlatformBridge());
  if (bridge == nullptr) {
    return;
  }
...
void Application::SetFocus(const std::shared_ptr<Widget>& widget) {
