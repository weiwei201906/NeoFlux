...
RenderLayer::~RenderLayer() {
  // Destructor must not throw; Stop() joins the render thread and logs.
  try {
    Stop();
  } catch (...) {
    // NOLINTNEXTLINE(bugprone-empty-catch) - swallowing is intentional:
    // destructors are noexcept and teardown failures must not propagate.
    (void)0;  // Best-effort teardown during destruction.
  }
}
...
void RenderLayer::RegisterMediaPlayer(MediaPlayer* player) {
  if (player == nullptr) {
    return;
  }
  std::scoped_lock<std::mutex> lock(media_players_mutex_);
  if (std::ranges::find(media_players_, player) == media_players_.end()) {
    media_players_.push_back(player);
    LOG(INFO) << "RenderLayer: registered media player";
  }
}

void RenderLayer::UnregisterMediaPlayer(MediaPlayer* player) {
  if (player == nullptr) {
    return;
  }
  std::scoped_lock<std::mutex> lock(media_players_mutex_);
  const auto it =
      std::ranges::find(media_players_, player);
  if (it != media_players_.end()) {
    media_players_.erase(it);
    LOG(INFO) << "RenderLayer: unregistered media player";
  }
}
...
#ifdef NEOFLUX_PLATFORM_DESKTOP
  if (platform_bridge_ != nullptr) {
    // dynamic_cast keeps RTTI safety; on desktop the bridge is always a
    // GlfwBridge when this branch is compiled.
    if (auto* glfw = dynamic_cast<GlfwBridge*>(platform_bridge_.get());
        glfw != nullptr) {
      glfw->GetWindowSize(width, height);
      return;
    }
  }
#endif
    ...
    while (running_.load() && command_queue_.TryPop(cmd)) {
      ...
      switch (cmd.type) {
        case RenderCommandType::kBeginFrame:
          ...
          // Refresh registered media players' video textures. This must run
          // on the render thread with a current GL context (mpv render
          // context creation fails otherwise: 'not supported').
          if (!media_players_.empty()) {
            std::scoped_lock<std::mutex> lock(media_players_mutex_);
            for (MediaPlayer* player : media_players_) {
              if (player != nullptr) {
                player->InitRender();
