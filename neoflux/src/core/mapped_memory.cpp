...
std::optional<MappedMemory> MappedMemory::FromFile(std::string_view path) {
  ...
  // string_view::data() is not guaranteed null-terminated; materialize a
  // std::string for the OS file APIs.
  const std::string path_str(path);

#ifdef _WIN32
  // Open file for reading.
  // Note: HANDLE is a typedef for void*; writing "const HANDLE" would yield
  // void* const (misc-misplaced-const), so declare the handle without const.
  HANDLE file_handle = CreateFileA(
      path_str.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
