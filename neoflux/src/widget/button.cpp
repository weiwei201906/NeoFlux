...
float EstimateLabelWidth(std::string_view label, float font_size) {
  ...
  for (std::size_t i = 0; i < label.size();) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - i < label.size() by loop invariant.
    const auto byte = static_cast<unsigned char>(label[i]);
    std::size_t char_len = 1;
    // Decode UTF-8 lead byte: 1-4 bytes per code point.
    // Bit masks: 0x80=continuation, 0xE0=2-byte, 0xF0=3-byte, 0xF8=4-byte.
    if ((byte & 0x80U) == 0U) {
      char_len = 1;
    ...
    width += (char_len > 1U) ? font_size : font_size * kLatinWidthRatio;
    i += char_len;
  }
  return width;
}
