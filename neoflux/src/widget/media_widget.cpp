...
//
// Threading model: the UI thread owns the MediaPlayer and drives playback
// control; the render thread refreshes the video texture every frame. The
// widget registers its player with RenderLayer, which calls InitRender() and
// UpdateTexture() on the render thread where the GL context is current.
// Paint() only reads the latest texture id (thread-safe) and issues draw
// commands.
// =============================================================================
...
MediaWidget::~MediaWidget() {
  if (player_ != nullptr) {
    player_->Stop();
    // Unregister from the render layer before the player is destroyed so the
    // render thread never touches a dangling player.
    Application* app = GetApplication();
    if (app != nullptr) {
      app->GetRenderLayer().UnregisterMediaPlayer(player_.get());
    }
  }
}
...
void MediaWidget::Paint(RenderContext& context) {
  ...
  // Register the player with the render layer on first paint so the render
  // thread can refresh its texture. Registration is idempotent.
  if (!registered_ && player_ != nullptr) {
    Application* app = GetApplication();
    if (app != nullptr) {
      app->GetRenderLayer().RegisterMediaPlayer(player_.get());
      registered_ = true;
    }
  }

  // Read the latest texture id (updated by the render thread) without
  // touching GL from the UI thread.
  if (player_ != nullptr) {
    current_texture_ = player_->GetTextureId();
