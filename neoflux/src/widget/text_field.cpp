...
bool TextField::OnPointerDown(const Point& local_pos) {
  ...
  if (rel_x <= 0.0F) {
    ...
    // lround is faster and more correct than a manual (x + 0.5) cast.
    // int32_t satisfies google-runtime-int; the value is small (chars in view).
    const auto rounded = static_cast<std::int32_t>(
        std::lround(rel_x / char_width));
    const auto target_char = static_cast<std::size_t>(std::max(rounded, 0));
    // Find byte offset for the target character.
    std::size_t char_count = 0;
    std::size_t byte_offset = 0;
    while (byte_offset < text_.size() && char_count < target_char) {
      // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - byte_offset < text_.size() by loop condition.
...
// NOLINTBEGIN(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - pos < text_.size() is guaranteed by the loop invariant (pos starts at from-1 < size and only decrements).
...
std::size_t TextField::NextCharOffset(std::size_t from) const noexcept {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - from < text_.size() by the guard above.
...
std::size_t TextField::Utf8CharCount(std::string_view text,
  ...
  for (std::size_t i = 0; i < limit;) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - i < limit <= text.size() by loop invariant.
