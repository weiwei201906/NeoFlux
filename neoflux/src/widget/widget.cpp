...
Size Widget::OnMeasure(float /*width*/, int /*width_mode*/,
  ...
  // Default: zero intrinsic size. Layout containers defer sizing to children.
  return {.width = 0.0F, .height = 0.0F};
}

// NOLINTBEGIN(misc-no-recursion) - widget tree traversal is inherently
// recursive; depth is bounded by the developer's own widget nesting.
void Widget::Paint(RenderContext& context) {
  // Default: paint children with appropriate translation.
...
// NOLINTBEGIN(misc-no-recursion) - propagates the app pointer down the
// widget tree; recursion depth equals widget nesting depth.
...
// NOLINTBEGIN(misc-no-recursion) - hit-test traversal of the widget tree;
// recursion depth equals widget nesting depth.
std::shared_ptr<Widget> Widget::HitTest(
  ...
  for (const auto& child : children_ | std::views::reverse) {
    ...
    // NOLINTNEXTLINE(misc-no-recursion) - hit-test traversal of the widget
    // tree; recursion depth equals widget nesting depth.
...
void Widget::SyncTaitankChildren() {
  ...
  for (std::size_t i = 0; i < children_.size(); ++i) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - loop bound guarantees i < size().
...
// NOLINTBEGIN(misc-no-recursion) - recursive read-back of Taitank layout
// bounds; recursion depth equals widget nesting depth.
void Widget::ReadLayoutRecursive() {
  ...
  for (auto& child : children_) {
    if (child != nullptr) {
      // NOLINTNEXTLINE(misc-no-recursion) - recursion depth equals widget
      // nesting depth.
...
// NOLINTBEGIN(misc-no-recursion) - recursive paint of child widgets;
// recursion depth equals widget nesting depth.
